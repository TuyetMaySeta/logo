...
export const EmailHeaderFilter = ({
  tableState,
  setTableState,
}: {
  ...
  return (
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      <PopoverTrigger asChild>
        ...
        >
          {t("employee.email.title")} {getFilterIcon()}
        ...
        <div className="flex flex-col gap-4 w-full">
          <div className="space-y-2">
            <h4 className="font-medium leading-none">
              {t("employee.email.header")}
            </h4>
            <p className="text-sm text-muted-foreground">
              {t("employee.email.description")}
            </p>
          </div>
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <Label className="text-sm font-medium">
                {t("employee.email.filter_label")}
              </Label>
              {currentFilter && (
                <Button
                  variant="ghost"
                ...
                >
                  <X className="h-3 w-3 mr-1 shrink-0" /> Xóa
                </Button>
              )}
            </div>
            <Input
              placeholder={t("employee.email.placeholder")}
              value={localFilter}
